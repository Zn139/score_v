<template>
  <div class="register-form tea">
    <group>
      <x-input title="姓名" v-model="form.teaName"></x-input>
      <x-input title="工号" v-model="form.teaCode"></x-input>
      <x-input title="科目" v-model="form.teaSubject"></x-input>
    </group>
    <group>
      <div class="password-control vux-cell-box" v-if="type==='reg'">
        <x-input v-if="!pwdVis" title="密码" type="password" v-model="form.teaPasswd"></x-input>
        <x-input v-if="pwdVis" title="密码" type="text" v-model="form.teaPasswd"></x-input>
...
export default {
  ...
  methods: {
    reg () {
      ...
      request({
        url: 'http://www.zhiheyikaoqin.cn/user/registerTeaBaseByOpenid',
        method: 'post',
        data: {
