  ...
  <div class="viewstu-block">
    ...
    <div class="viewstu__body">
      ...
      <panel v-if="!loading && stuList.length !==0" :list="stuList" :type="'5'" @on-click-item="selStu">
        <div class="sel-stu" slot="flag" slot-scope="item" v-if="item.item.selected">选中</div>
      </panel>
...
export default {
  ...
  methods: {
    _getStu () {
      ...
      this.selStuId = ''
      request({
        url: 'http://www.zhiheyikaoqin.cn/tea/findCandidatesByCourseId?courseId=' + this.courseId + '&teaOpenid=' + this.teaOpenid,
        method: 'get'
      }).then(res => {
        this.loading = false
        if (res.data.code !== 0) {
    ...
    _selAPI (item) {
      request({
        url: 'http://www.zhiheyikaoqin.cn/tea/saveSelectedStu',
        method: 'post',
        data: {
